...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distance_val = self.get_euclidean_distance(self.centroids.iloc[j], sliced_data)
                distances[j] = distance_val
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.DataFrame) or isinstance(point2, pd.DataFrame):
            euc_dist_df = np.sqrt(((point2-point1)**2).sum(axis = 1))
            return euc_dist_df
        elif isinstance(point1, pd.Series) or isinstance(point2, pd.Series):
            euc_dist_series = np.sqrt(pd.Series((point2-point1)**2).sum())
            return euc_dist_series
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            distance_val_updated = self.get_euclidean_distance(self.centroids.iloc[i], sliced_data)
            distances[i] = distance_val_updated
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            # data points.
            if (cur_groups.equals(groups) == True):
                flag_groups = True

            # TODO: Check if there are changes with the values of the centroids
            if (self.centroids.equals(centroids) == True):
                flag_centroids = True
